...
---
> 2022/11/3 更新2.0版本： 
> 1. 新增合併功能，可以將多個匯出的題目檔合併
> 2. 程式開始執行前會先詢問是否開啟翻譯功能
> 3. 讀檔後程式會在原位置開啟（而不是螢幕中間）
> 4. MacOS實測可以透過直接執行Python檔執行，沒有跑版問題，各功能可以順利運行
>
> 以上詳細說明等我考完期中考再寫 ，最近好忙
...
2. 透過下圖的下拉式選單選擇要練習的章節，或選擇「所有章節」一次性匯入該檔案中所有題目，此時可以勾選右邊「跳過是非」、「順序隨機」來略過是非題或打散題目順序。<br>
![匯入檔案](imgs/load_question.PNG)
3. 選擇章節與匯入設定後，按下`Change LO`按鈕開始匯入題目。
4. 按下對應的ＡＢＣＤ按紐來答題，亦可以透過1234來選擇ＡＢＣＤ。是非題中1代表False，2代表True。
5. 按下enter或右方向鍵可以到下一題，按下左方向鍵則可回到上一題。
6. 做題過程中可以隨時框選題目中的文字，將會自動帶入Chrome中。或在題目區中按下滑鼠右鍵點擊「翻譯」則可翻譯整個題目。（框選兩三個單字時可能會變成整段選取，可以調整timeout的時間，或翻譯整個題目後，再在google翻譯中選取）
7. **注意：若過程中不慎將google翻譯關閉，將導致程式暫時沒有回應，稍待片刻後程式即恢復正常，但瀏覽器將不會再重新啟動，並不再支援翻譯功能（須將程式重啟）**
8. 若答題時對於該題不滿意，可以按下`Redo`按鈕，程式將打散選項順序，並且重置選項按鈕功能。
9. 作答過程中（或已完成作答）可以使用file>save as將做錯的題目匯出為一個json檔，並在未來使用load from功能重新開啟
10. **若有不熟悉的題目希望標記，請使用redo功能手動做錯標記**
